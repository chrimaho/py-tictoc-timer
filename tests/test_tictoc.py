...
class TestTicToc(TestCase):
    ...
    def test_tictoc_restart(self) -> None:
        tt = TicToc()
        tt.tick()
        sleep(1.1)
        tt.toc(restart=True)
    ...
    def test_tictoc_value(self) -> None:
        tt = TicToc()
        tt.tic()
        sleep(1.1)
    ...
    def test_tictoc_string(self) -> None:
        tt = TicToc()
        tt.tic()
        sleep(1.1)
        val = tt.toc_string()
    ...
    def test_tictoc_extended(self) -> None:
        with TicToc():
            sleep(75.1)
        captured = self.capsys.readouterr()
        self.assertEqual("Elapsed time: 1mins 15secs\n", captured.out)

    def test_tictoc_raises(self) -> None:
        tt = TicToc()
        print(tt.start)
